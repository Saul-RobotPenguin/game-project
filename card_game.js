...
//through the ranks.The inner loop should loop through the suits.
//Make sure to use different variables for your iterators.
// 6. inside your inner for loop, push your looped iterations of ranks and suits as OBJECTS into the empty deck
//array.Add a third property to this object with the key "value"
...
function buildDeck() {
  ...
  for (let i = 0; i < suits.length; i++) {
    for (let j = 0; j < ranks.length; j++) {
      let card = { Ranks: ranks[j], Suits: suits[i] };
      deck.push(card);
    }
  }
  return deck;
...
function greet() {
  let name = getInput(
    "Hello and welcome to my game. Could you be kind as to tell me whats your name?"
  );
  console.log(name);
  return name;
}
...
function compare(card1, card2) {
  let card1And2Result = card1.value - card2.value;
  return card1And2Result;
}
...
// 3. declare a variable called "input" that uses getInput() to ask the user if they think the next card
// will be higher(h) or lower(l) than their current card and stores the user's response.
...
// 7. If input doesn't equal h or l, tell the user that they need to guess either h or l and that they
//get no points for this round, then return false.

function guess(card1, card2) {
  console.log(card1.rank, card1.suit);
  let input = getInput(
    "What do you think the next card is going to be, higher (type h) or lower ( type l) than your current card??"
  );
  if (input === "h") {
    ...
    console.log(
      "You need to guess either h or l so unfortunatly you won't receive any points for this round"
    );
    return false;
  }
}
...
// 2. declare a variable called deck (it's okay to reuse -- remember scope!) that takes the result of
//the shuffle function. Remember that the shuffle function needs to take the results one of our other functions
// as its argument...
...
// 7. Inside the while loop, use an array method on deck to remove the last object and assign that
//value to a variable named nextCard.
// 8. Inside the while loop, create a conditional statement. If the outcome of guess is true,
//increment the score by 1, congratulate the user, and tell them their score.If it's false, tell them
//they were wrong and got no points.
// 9. Close the conditional statement and assign nextCard to currentCard. You may have to write this as
//the type of variable that's always global...
// 10. Close the while loop and use a ternary statement that checks if the length of the deck array
//has reached zero.If it has not, tell the user that they won.If it has reached zero, tell them that
//they're out of cards and they lost.
...
function playGame() {
  ...
  deck.length === 0
    ? playerName + " you are out of cards, you lose"
    : playerName + " You win!";
}
playGame();
